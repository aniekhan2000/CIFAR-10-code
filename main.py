...
#############################svae results ##################
"""
FLGuardian Modular — Main Script (Trust-Calibrated Detection + Metric Logging)
------------------------------------------------------------
Runs the full federated learning pipeline:
- Data loading
- Local training (with optional malicious clients)
...
- Aggregation and evaluation
- Dynamic trust-based detection
- Saves per-round metrics and averages to CSV
"""

import os, sys, time, copy, numpy as np, torch, csv
...
def run():
    ...
    save_path = "/content/metrics_log.csv"

    header = ["Round", "Global_Acc", "Precision", "Recall", "F1", "TP", "FP", "TN", "FN"]
    rows = [
        [rnd+1, all_acc[rnd], all_prec[rnd], all_rec[rnd], all_f1[rnd],
         all_tp[rnd], all_fp[rnd], all_tn[rnd], all_fn[rnd]]
        for rnd in range(len(all_acc))
    ]
    rows.append(["Average", avg_acc, avg_prec, avg_rec, avg_f1, avg_TP, avg_FP, avg_TN, avg_FN])
    ...
    np.savetxt("/content/global_acc_per_round.txt", global_acc_per_round, fmt="%.4f")
    print(f"\n💾 Metrics saved successfully to {save_path} and global_acc_per_round.txt")
